...
setup(
    ...
    entry_points={
        'console_scripts': [
            CODE_DIRECTORY+'= '+CODE_DIRECTORY+'.main:entry_point'
        ]
    }
)
# [[[endsection]]]
...
options(
    deploy = Bunch(
        htmldir = path(CODE_DIRECTORY+'/docs'),
        hosts = ['host1.hostymost.com', 'host2.hostymost.com'],
        hostpath = 'sites/'+CODE_DIRECTORY
    )
)
# [[[endsection]]]
...
@cmdopts([
    ...
    ('hostpath=', 'p', 'hostpath to use when directory on the server')
])
def deploy(options):
