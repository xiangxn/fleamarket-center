# -*- coding: utf-8 -*-
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: bitsflea.proto

from google.protobuf import descriptor as _descriptor
from google.protobuf import message as _message
from google.protobuf import reflection as _reflection
from google.protobuf import symbol_database as _symbol_database
# @@protoc_insertion_point(imports)

_sym_db = _symbol_database.Default()


from google.api import annotations_pb2 as google_dot_api_dot_annotations__pb2


DESCRIPTOR = _descriptor.FileDescriptor(
  name='bitsflea.proto',
  package='bitsflea',
  syntax='proto3',
  serialized_options=None,
  serialized_pb=b'\n\x0e\x62itsflea.proto\x12\x08\x62itsflea\x1a\x1cgoogle/api/annotations.proto\"&\n\tBaseReply\x12\x0c\n\x04\x63ode\x18\x01 \x01(\x05\x12\x0b\n\x03msg\x18\x02 \x01(\t\"@\n\x0bSearchReply\x12#\n\x06status\x18\x01 \x01(\x0b\x32\x13.bitsflea.BaseReply\x12\x0c\n\x04\x64\x61ta\x18\x02 \x01(\t\"@\n\nTokenReply\x12#\n\x06status\x18\x01 \x01(\x0b\x32\x13.bitsflea.BaseReply\x12\r\n\x05token\x18\x02 \x01(\t\")\n\x0b\x46ileRequest\x12\x0c\n\x04\x66ile\x18\x01 \x01(\x0c\x12\x0c\n\x04name\x18\x02 \x01(\t\"\x1d\n\x0c\x45osidRequest\x12\r\n\x05\x65osid\x18\x01 \x01(\t\"\xd5\x02\n\x04User\x12\x0e\n\x06userid\x18\x01 \x01(\x05\x12\r\n\x05\x65osid\x18\x02 \x01(\t\x12\r\n\x05phone\x18\x03 \x01(\t\x12\x0e\n\x06status\x18\x04 \x01(\x05\x12\x10\n\x08nickname\x18\x05 \x01(\t\x12\x0c\n\x04head\x18\x06 \x01(\t\x12\x13\n\x0b\x63reditValue\x18\x07 \x01(\x05\x12\x10\n\x08referrer\x18\x08 \x01(\t\x12\x16\n\x0elastActiveTime\x18\t \x01(\t\x12\x12\n\npostsTotal\x18\n \x01(\x05\x12\x11\n\tsellTotal\x18\x0b \x01(\x05\x12\x10\n\x08\x62uyTotal\x18\x0c \x01(\x05\x12\x15\n\rreferralTotal\x18\r \x01(\x05\x12\r\n\x05point\x18\x0e \x01(\t\x12\x12\n\nisReviewer\x18\x0f \x01(\x05\x12\x13\n\x0b\x66ollowTotal\x18\x10 \x01(\x05\x12\x15\n\rfavoriteTotal\x18\x11 \x01(\x05\x12\x11\n\tfansTotal\x18\x12 \x01(\x05\"\xa4\x01\n\x0fRegisterRequest\x12\r\n\x05phone\x18\x01 \x01(\t\x12\x10\n\x08nickname\x18\x02 \x01(\t\x12\x13\n\x0bownerpubkey\x18\x03 \x01(\t\x12\x11\n\tactpubkey\x18\x04 \x01(\t\x12\x0f\n\x07smscode\x18\x05 \x01(\t\x12\x10\n\x08referral\x18\x06 \x01(\t\x12\x0f\n\x07\x61uthkey\x18\x07 \x01(\t\x12\x14\n\x0cphoneEncrypt\x18\x08 \x01(\t\"R\n\rRegisterReply\x12#\n\x06status\x18\x01 \x01(\x0b\x32\x13.bitsflea.BaseReply\x12\x1c\n\x04\x64\x61ta\x18\x02 \x01(\x0b\x32\x0e.bitsflea.User\"-\n\nSmsRequest\x12\r\n\x05phone\x18\x01 \x01(\t\x12\x10\n\x08\x63odeType\x18\x02 \x01(\x05\"O\n\x13RefreshTokenRequest\x12\r\n\x05phone\x18\x01 \x01(\t\x12\r\n\x05token\x18\x02 \x01(\t\x12\x0c\n\x04time\x18\x03 \x01(\x05\x12\x0c\n\x04sign\x18\x04 \x01(\t\"\x1e\n\rSearchRequest\x12\r\n\x05query\x18\x01 \x01(\t\"/\n\x12TransactionRequest\x12\x0b\n\x03trx\x18\x01 \x01(\t\x12\x0c\n\x04sign\x18\x02 \x01(\x05\"/\n\rFollowRequest\x12\x0c\n\x04user\x18\x01 \x01(\x05\x12\x10\n\x08\x66ollower\x18\x02 \x01(\x05\"0\n\x0f\x46\x61voriteRequest\x12\x0c\n\x04user\x18\x01 \x01(\x05\x12\x0f\n\x07product\x18\x02 \x01(\x05\"\xb0\x01\n\x0e\x41\x64\x64ressRequest\x12\x0b\n\x03rid\x18\x01 \x01(\x05\x12\x0e\n\x06userid\x18\x02 \x01(\x05\x12\x10\n\x08province\x18\x03 \x01(\t\x12\x0c\n\x04\x63ity\x18\x04 \x01(\t\x12\x10\n\x08\x64istrict\x18\x05 \x01(\t\x12\r\n\x05phone\x18\x06 \x01(\t\x12\x0c\n\x04name\x18\x07 \x01(\t\x12\x0f\n\x07\x61\x64\x64ress\x18\x08 \x01(\t\x12\x10\n\x08postcode\x18\t \x01(\t\x12\x0f\n\x07\x64\x65\x66\x61ult\x18\n \x01(\x05\"4\n\x15SetDefaultAddrRequest\x12\x0e\n\x06userid\x18\x01 \x01(\x05\x12\x0b\n\x03rid\x18\x02 \x01(\x05\x32\xf0\n\n\x08\x42itsFlea\x12^\n\x08Register\x12\x19.bitsflea.RegisterRequest\x1a\x17.bitsflea.RegisterReply\"\x1e\x82\xd3\xe4\x93\x02\x18\"\x13/v1/common/register:\x01*\x12[\n\x0bSendSmsCode\x12\x14.bitsflea.SmsRequest\x1a\x13.bitsflea.BaseReply\"!\x82\xd3\xe4\x93\x02\x1b\"\x16/v1/common/sendSmsCode:\x01*\x12g\n\x0cRefreshToken\x12\x1d.bitsflea.RefreshTokenRequest\x1a\x14.bitsflea.TokenReply\"\"\x82\xd3\xe4\x93\x02\x1c\"\x17/v1/common/refreshtoken:\x01*\x12W\n\x08Referral\x12\x16.bitsflea.EosidRequest\x1a\x13.bitsflea.BaseReply\"\x1e\x82\xd3\xe4\x93\x02\x18\"\x13/v1/common/Referral:\x01*\x12V\n\x06Search\x12\x17.bitsflea.SearchRequest\x1a\x15.bitsflea.SearchReply\"\x1c\x82\xd3\xe4\x93\x02\x16\"\x11/v1/common/search:\x01*\x12\x63\n\x0bTransaction\x12\x1c.bitsflea.TransactionRequest\x1a\x13.bitsflea.BaseReply\"!\x82\xd3\xe4\x93\x02\x1b\"\x16/v1/common/transaction:\x01*\x12T\n\x06\x46ollow\x12\x17.bitsflea.FollowRequest\x1a\x13.bitsflea.BaseReply\"\x1c\x82\xd3\xe4\x93\x02\x16\"\x11/v1/common/follow:\x01*\x12X\n\x08UnFollow\x12\x17.bitsflea.FollowRequest\x1a\x13.bitsflea.BaseReply\"\x1e\x82\xd3\xe4\x93\x02\x18\"\x13/v1/common/unfollow:\x01*\x12Z\n\x08\x46\x61vorite\x12\x19.bitsflea.FavoriteRequest\x1a\x13.bitsflea.BaseReply\"\x1e\x82\xd3\xe4\x93\x02\x18\"\x13/v1/common/favorite:\x01*\x12]\n\tUnCollect\x12\x19.bitsflea.FavoriteRequest\x1a\x13.bitsflea.BaseReply\" \x82\xd3\xe4\x93\x02\x1a\"\x15/v1/common/unfavorite:\x01*\x12W\n\x07\x41\x64\x64ress\x12\x18.bitsflea.AddressRequest\x1a\x13.bitsflea.BaseReply\"\x1d\x82\xd3\xe4\x93\x02\x17\"\x12/v1/common/address:\x01*\x12\x63\n\rUpdateAddress\x12\x18.bitsflea.AddressRequest\x1a\x13.bitsflea.BaseReply\"#\x82\xd3\xe4\x93\x02\x1d\"\x18/v1/common/updateaddress:\x01*\x12]\n\nDelAddress\x12\x18.bitsflea.AddressRequest\x1a\x13.bitsflea.BaseReply\" \x82\xd3\xe4\x93\x02\x1a\"\x15/v1/common/deladdress:\x01*\x12h\n\x0eSetDefaultAddr\x12\x1f.bitsflea.SetDefaultAddrRequest\x1a\x13.bitsflea.BaseReply\" \x82\xd3\xe4\x93\x02\x1a\"\x15/v1/common/setdefault:\x01*\x12\x36\n\x06Upload\x12\x15.bitsflea.FileRequest\x1a\x13.bitsflea.BaseReply\"\x00\x62\x06proto3'
  ,
  dependencies=[google_dot_api_dot_annotations__pb2.DESCRIPTOR,])




_BASEREPLY = _descriptor.Descriptor(
  name='BaseReply',
  full_name='bitsflea.BaseReply',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='code', full_name='bitsflea.BaseReply.code', index=0,
      number=1, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='msg', full_name='bitsflea.BaseReply.msg', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=58,
  serialized_end=96,
)


_SEARCHREPLY = _descriptor.Descriptor(
  name='SearchReply',
  full_name='bitsflea.SearchReply',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='status', full_name='bitsflea.SearchReply.status', index=0,
      number=1, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='data', full_name='bitsflea.SearchReply.data', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=98,
  serialized_end=162,
)


_TOKENREPLY = _descriptor.Descriptor(
  name='TokenReply',
  full_name='bitsflea.TokenReply',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='status', full_name='bitsflea.TokenReply.status', index=0,
      number=1, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='token', full_name='bitsflea.TokenReply.token', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=164,
  serialized_end=228,
)


_FILEREQUEST = _descriptor.Descriptor(
  name='FileRequest',
  full_name='bitsflea.FileRequest',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='file', full_name='bitsflea.FileRequest.file', index=0,
      number=1, type=12, cpp_type=9, label=1,
      has_default_value=False, default_value=b"",
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='name', full_name='bitsflea.FileRequest.name', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=230,
  serialized_end=271,
)


_EOSIDREQUEST = _descriptor.Descriptor(
  name='EosidRequest',
  full_name='bitsflea.EosidRequest',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='eosid', full_name='bitsflea.EosidRequest.eosid', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=273,
  serialized_end=302,
)


_USER = _descriptor.Descriptor(
  name='User',
  full_name='bitsflea.User',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='userid', full_name='bitsflea.User.userid', index=0,
      number=1, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='eosid', full_name='bitsflea.User.eosid', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='phone', full_name='bitsflea.User.phone', index=2,
      number=3, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='status', full_name='bitsflea.User.status', index=3,
      number=4, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='nickname', full_name='bitsflea.User.nickname', index=4,
      number=5, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='head', full_name='bitsflea.User.head', index=5,
      number=6, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='creditValue', full_name='bitsflea.User.creditValue', index=6,
      number=7, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='referrer', full_name='bitsflea.User.referrer', index=7,
      number=8, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='lastActiveTime', full_name='bitsflea.User.lastActiveTime', index=8,
      number=9, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='postsTotal', full_name='bitsflea.User.postsTotal', index=9,
      number=10, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='sellTotal', full_name='bitsflea.User.sellTotal', index=10,
      number=11, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='buyTotal', full_name='bitsflea.User.buyTotal', index=11,
      number=12, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='referralTotal', full_name='bitsflea.User.referralTotal', index=12,
      number=13, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='point', full_name='bitsflea.User.point', index=13,
      number=14, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='isReviewer', full_name='bitsflea.User.isReviewer', index=14,
      number=15, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='followTotal', full_name='bitsflea.User.followTotal', index=15,
      number=16, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='favoriteTotal', full_name='bitsflea.User.favoriteTotal', index=16,
      number=17, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='fansTotal', full_name='bitsflea.User.fansTotal', index=17,
      number=18, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=305,
  serialized_end=646,
)


_REGISTERREQUEST = _descriptor.Descriptor(
  name='RegisterRequest',
  full_name='bitsflea.RegisterRequest',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='phone', full_name='bitsflea.RegisterRequest.phone', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='nickname', full_name='bitsflea.RegisterRequest.nickname', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='ownerpubkey', full_name='bitsflea.RegisterRequest.ownerpubkey', index=2,
      number=3, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='actpubkey', full_name='bitsflea.RegisterRequest.actpubkey', index=3,
      number=4, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='smscode', full_name='bitsflea.RegisterRequest.smscode', index=4,
      number=5, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='referral', full_name='bitsflea.RegisterRequest.referral', index=5,
      number=6, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='authkey', full_name='bitsflea.RegisterRequest.authkey', index=6,
      number=7, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='phoneEncrypt', full_name='bitsflea.RegisterRequest.phoneEncrypt', index=7,
      number=8, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=649,
  serialized_end=813,
)


_REGISTERREPLY = _descriptor.Descriptor(
  name='RegisterReply',
  full_name='bitsflea.RegisterReply',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='status', full_name='bitsflea.RegisterReply.status', index=0,
      number=1, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='data', full_name='bitsflea.RegisterReply.data', index=1,
      number=2, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=815,
  serialized_end=897,
)


_SMSREQUEST = _descriptor.Descriptor(
  name='SmsRequest',
  full_name='bitsflea.SmsRequest',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='phone', full_name='bitsflea.SmsRequest.phone', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='codeType', full_name='bitsflea.SmsRequest.codeType', index=1,
      number=2, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=899,
  serialized_end=944,
)


_REFRESHTOKENREQUEST = _descriptor.Descriptor(
  name='RefreshTokenRequest',
  full_name='bitsflea.RefreshTokenRequest',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='phone', full_name='bitsflea.RefreshTokenRequest.phone', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='token', full_name='bitsflea.RefreshTokenRequest.token', index=1,
      number=2, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='time', full_name='bitsflea.RefreshTokenRequest.time', index=2,
      number=3, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='sign', full_name='bitsflea.RefreshTokenRequest.sign', index=3,
      number=4, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=946,
  serialized_end=1025,
)


_SEARCHREQUEST = _descriptor.Descriptor(
  name='SearchRequest',
  full_name='bitsflea.SearchRequest',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='query', full_name='bitsflea.SearchRequest.query', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=1027,
  serialized_end=1057,
)


_TRANSACTIONREQUEST = _descriptor.Descriptor(
  name='TransactionRequest',
  full_name='bitsflea.TransactionRequest',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='trx', full_name='bitsflea.TransactionRequest.trx', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='sign', full_name='bitsflea.TransactionRequest.sign', index=1,
      number=2, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=1059,
  serialized_end=1106,
)


_FOLLOWREQUEST = _descriptor.Descriptor(
  name='FollowRequest',
  full_name='bitsflea.FollowRequest',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='user', full_name='bitsflea.FollowRequest.user', index=0,
      number=1, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='follower', full_name='bitsflea.FollowRequest.follower', index=1,
      number=2, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=1108,
  serialized_end=1155,
)


_FAVORITEREQUEST = _descriptor.Descriptor(
  name='FavoriteRequest',
  full_name='bitsflea.FavoriteRequest',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='user', full_name='bitsflea.FavoriteRequest.user', index=0,
      number=1, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='product', full_name='bitsflea.FavoriteRequest.product', index=1,
      number=2, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=1157,
  serialized_end=1205,
)


_ADDRESSREQUEST = _descriptor.Descriptor(
  name='AddressRequest',
  full_name='bitsflea.AddressRequest',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='rid', full_name='bitsflea.AddressRequest.rid', index=0,
      number=1, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='userid', full_name='bitsflea.AddressRequest.userid', index=1,
      number=2, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='province', full_name='bitsflea.AddressRequest.province', index=2,
      number=3, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='city', full_name='bitsflea.AddressRequest.city', index=3,
      number=4, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='district', full_name='bitsflea.AddressRequest.district', index=4,
      number=5, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='phone', full_name='bitsflea.AddressRequest.phone', index=5,
      number=6, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='name', full_name='bitsflea.AddressRequest.name', index=6,
      number=7, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='address', full_name='bitsflea.AddressRequest.address', index=7,
      number=8, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='postcode', full_name='bitsflea.AddressRequest.postcode', index=8,
      number=9, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=b"".decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='default', full_name='bitsflea.AddressRequest.default', index=9,
      number=10, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=1208,
  serialized_end=1384,
)


_SETDEFAULTADDRREQUEST = _descriptor.Descriptor(
  name='SetDefaultAddrRequest',
  full_name='bitsflea.SetDefaultAddrRequest',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='userid', full_name='bitsflea.SetDefaultAddrRequest.userid', index=0,
      number=1, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='rid', full_name='bitsflea.SetDefaultAddrRequest.rid', index=1,
      number=2, type=5, cpp_type=1, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      serialized_options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  serialized_options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
  ],
  serialized_start=1386,
  serialized_end=1438,
)

_SEARCHREPLY.fields_by_name['status'].message_type = _BASEREPLY
_TOKENREPLY.fields_by_name['status'].message_type = _BASEREPLY
_REGISTERREPLY.fields_by_name['status'].message_type = _BASEREPLY
_REGISTERREPLY.fields_by_name['data'].message_type = _USER
DESCRIPTOR.message_types_by_name['BaseReply'] = _BASEREPLY
DESCRIPTOR.message_types_by_name['SearchReply'] = _SEARCHREPLY
DESCRIPTOR.message_types_by_name['TokenReply'] = _TOKENREPLY
DESCRIPTOR.message_types_by_name['FileRequest'] = _FILEREQUEST
DESCRIPTOR.message_types_by_name['EosidRequest'] = _EOSIDREQUEST
DESCRIPTOR.message_types_by_name['User'] = _USER
DESCRIPTOR.message_types_by_name['RegisterRequest'] = _REGISTERREQUEST
DESCRIPTOR.message_types_by_name['RegisterReply'] = _REGISTERREPLY
DESCRIPTOR.message_types_by_name['SmsRequest'] = _SMSREQUEST
DESCRIPTOR.message_types_by_name['RefreshTokenRequest'] = _REFRESHTOKENREQUEST
DESCRIPTOR.message_types_by_name['SearchRequest'] = _SEARCHREQUEST
DESCRIPTOR.message_types_by_name['TransactionRequest'] = _TRANSACTIONREQUEST
DESCRIPTOR.message_types_by_name['FollowRequest'] = _FOLLOWREQUEST
DESCRIPTOR.message_types_by_name['FavoriteRequest'] = _FAVORITEREQUEST
DESCRIPTOR.message_types_by_name['AddressRequest'] = _ADDRESSREQUEST
DESCRIPTOR.message_types_by_name['SetDefaultAddrRequest'] = _SETDEFAULTADDRREQUEST
_sym_db.RegisterFileDescriptor(DESCRIPTOR)

BaseReply = _reflection.GeneratedProtocolMessageType('BaseReply', (_message.Message,), {
  'DESCRIPTOR' : _BASEREPLY,
  '__module__' : 'bitsflea_pb2'
  # @@protoc_insertion_point(class_scope:bitsflea.BaseReply)
  })
_sym_db.RegisterMessage(BaseReply)

SearchReply = _reflection.GeneratedProtocolMessageType('SearchReply', (_message.Message,), {
  'DESCRIPTOR' : _SEARCHREPLY,
  '__module__' : 'bitsflea_pb2'
  # @@protoc_insertion_point(class_scope:bitsflea.SearchReply)
  })
_sym_db.RegisterMessage(SearchReply)

TokenReply = _reflection.GeneratedProtocolMessageType('TokenReply', (_message.Message,), {
  'DESCRIPTOR' : _TOKENREPLY,
  '__module__' : 'bitsflea_pb2'
  # @@protoc_insertion_point(class_scope:bitsflea.TokenReply)
  })
_sym_db.RegisterMessage(TokenReply)

FileRequest = _reflection.GeneratedProtocolMessageType('FileRequest', (_message.Message,), {
  'DESCRIPTOR' : _FILEREQUEST,
  '__module__' : 'bitsflea_pb2'
  # @@protoc_insertion_point(class_scope:bitsflea.FileRequest)
  })
_sym_db.RegisterMessage(FileRequest)

EosidRequest = _reflection.GeneratedProtocolMessageType('EosidRequest', (_message.Message,), {
  'DESCRIPTOR' : _EOSIDREQUEST,
  '__module__' : 'bitsflea_pb2'
  # @@protoc_insertion_point(class_scope:bitsflea.EosidRequest)
  })
_sym_db.RegisterMessage(EosidRequest)

User = _reflection.GeneratedProtocolMessageType('User', (_message.Message,), {
  'DESCRIPTOR' : _USER,
  '__module__' : 'bitsflea_pb2'
  # @@protoc_insertion_point(class_scope:bitsflea.User)
  })
_sym_db.RegisterMessage(User)

RegisterRequest = _reflection.GeneratedProtocolMessageType('RegisterRequest', (_message.Message,), {
  'DESCRIPTOR' : _REGISTERREQUEST,
  '__module__' : 'bitsflea_pb2'
  # @@protoc_insertion_point(class_scope:bitsflea.RegisterRequest)
  })
_sym_db.RegisterMessage(RegisterRequest)

RegisterReply = _reflection.GeneratedProtocolMessageType('RegisterReply', (_message.Message,), {
  'DESCRIPTOR' : _REGISTERREPLY,
  '__module__' : 'bitsflea_pb2'
  # @@protoc_insertion_point(class_scope:bitsflea.RegisterReply)
  })
_sym_db.RegisterMessage(RegisterReply)

SmsRequest = _reflection.GeneratedProtocolMessageType('SmsRequest', (_message.Message,), {
  'DESCRIPTOR' : _SMSREQUEST,
  '__module__' : 'bitsflea_pb2'
  # @@protoc_insertion_point(class_scope:bitsflea.SmsRequest)
  })
_sym_db.RegisterMessage(SmsRequest)

RefreshTokenRequest = _reflection.GeneratedProtocolMessageType('RefreshTokenRequest', (_message.Message,), {
  'DESCRIPTOR' : _REFRESHTOKENREQUEST,
  '__module__' : 'bitsflea_pb2'
  # @@protoc_insertion_point(class_scope:bitsflea.RefreshTokenRequest)
  })
_sym_db.RegisterMessage(RefreshTokenRequest)

SearchRequest = _reflection.GeneratedProtocolMessageType('SearchRequest', (_message.Message,), {
  'DESCRIPTOR' : _SEARCHREQUEST,
  '__module__' : 'bitsflea_pb2'
  # @@protoc_insertion_point(class_scope:bitsflea.SearchRequest)
  })
_sym_db.RegisterMessage(SearchRequest)

TransactionRequest = _reflection.GeneratedProtocolMessageType('TransactionRequest', (_message.Message,), {
  'DESCRIPTOR' : _TRANSACTIONREQUEST,
  '__module__' : 'bitsflea_pb2'
  # @@protoc_insertion_point(class_scope:bitsflea.TransactionRequest)
  })
_sym_db.RegisterMessage(TransactionRequest)

FollowRequest = _reflection.GeneratedProtocolMessageType('FollowRequest', (_message.Message,), {
  'DESCRIPTOR' : _FOLLOWREQUEST,
  '__module__' : 'bitsflea_pb2'
  # @@protoc_insertion_point(class_scope:bitsflea.FollowRequest)
  })
_sym_db.RegisterMessage(FollowRequest)

FavoriteRequest = _reflection.GeneratedProtocolMessageType('FavoriteRequest', (_message.Message,), {
  'DESCRIPTOR' : _FAVORITEREQUEST,
  '__module__' : 'bitsflea_pb2'
  # @@protoc_insertion_point(class_scope:bitsflea.FavoriteRequest)
  })
_sym_db.RegisterMessage(FavoriteRequest)

AddressRequest = _reflection.GeneratedProtocolMessageType('AddressRequest', (_message.Message,), {
  'DESCRIPTOR' : _ADDRESSREQUEST,
  '__module__' : 'bitsflea_pb2'
  # @@protoc_insertion_point(class_scope:bitsflea.AddressRequest)
  })
_sym_db.RegisterMessage(AddressRequest)

SetDefaultAddrRequest = _reflection.GeneratedProtocolMessageType('SetDefaultAddrRequest', (_message.Message,), {
  'DESCRIPTOR' : _SETDEFAULTADDRREQUEST,
  '__module__' : 'bitsflea_pb2'
  # @@protoc_insertion_point(class_scope:bitsflea.SetDefaultAddrRequest)
  })
_sym_db.RegisterMessage(SetDefaultAddrRequest)



_BITSFLEA = _descriptor.ServiceDescriptor(
  name='BitsFlea',
  full_name='bitsflea.BitsFlea',
  file=DESCRIPTOR,
  index=0,
  serialized_options=None,
  serialized_start=1441,
  serialized_end=2833,
  methods=[
  _descriptor.MethodDescriptor(
    name='Register',
    full_name='bitsflea.BitsFlea.Register',
    index=0,
    containing_service=None,
    input_type=_REGISTERREQUEST,
    output_type=_REGISTERREPLY,
    serialized_options=b'\202\323\344\223\002\030\"\023/v1/common/register:\001*',
  ),
  _descriptor.MethodDescriptor(
    name='SendSmsCode',
    full_name='bitsflea.BitsFlea.SendSmsCode',
    index=1,
    containing_service=None,
    input_type=_SMSREQUEST,
    output_type=_BASEREPLY,
    serialized_options=b'\202\323\344\223\002\033\"\026/v1/common/sendSmsCode:\001*',
  ),
  _descriptor.MethodDescriptor(
    name='RefreshToken',
    full_name='bitsflea.BitsFlea.RefreshToken',
    index=2,
    containing_service=None,
    input_type=_REFRESHTOKENREQUEST,
    output_type=_TOKENREPLY,
    serialized_options=b'\202\323\344\223\002\034\"\027/v1/common/refreshtoken:\001*',
  ),
  _descriptor.MethodDescriptor(
    name='Referral',
    full_name='bitsflea.BitsFlea.Referral',
    index=3,
    containing_service=None,
    input_type=_EOSIDREQUEST,
    output_type=_BASEREPLY,
    serialized_options=b'\202\323\344\223\002\030\"\023/v1/common/Referral:\001*',
  ),
  _descriptor.MethodDescriptor(
    name='Search',
    full_name='bitsflea.BitsFlea.Search',
    index=4,
    containing_service=None,
    input_type=_SEARCHREQUEST,
    output_type=_SEARCHREPLY,
    serialized_options=b'\202\323\344\223\002\026\"\021/v1/common/search:\001*',
  ),
  _descriptor.MethodDescriptor(
    name='Transaction',
    full_name='bitsflea.BitsFlea.Transaction',
    index=5,
    containing_service=None,
    input_type=_TRANSACTIONREQUEST,
    output_type=_BASEREPLY,
    serialized_options=b'\202\323\344\223\002\033\"\026/v1/common/transaction:\001*',
  ),
  _descriptor.MethodDescriptor(
    name='Follow',
    full_name='bitsflea.BitsFlea.Follow',
    index=6,
    containing_service=None,
    input_type=_FOLLOWREQUEST,
    output_type=_BASEREPLY,
    serialized_options=b'\202\323\344\223\002\026\"\021/v1/common/follow:\001*',
  ),
  _descriptor.MethodDescriptor(
    name='UnFollow',
    full_name='bitsflea.BitsFlea.UnFollow',
    index=7,
    containing_service=None,
    input_type=_FOLLOWREQUEST,
    output_type=_BASEREPLY,
    serialized_options=b'\202\323\344\223\002\030\"\023/v1/common/unfollow:\001*',
  ),
  _descriptor.MethodDescriptor(
    name='Favorite',
    full_name='bitsflea.BitsFlea.Favorite',
    index=8,
    containing_service=None,
    input_type=_FAVORITEREQUEST,
    output_type=_BASEREPLY,
    serialized_options=b'\202\323\344\223\002\030\"\023/v1/common/favorite:\001*',
  ),
  _descriptor.MethodDescriptor(
    name='UnCollect',
    full_name='bitsflea.BitsFlea.UnCollect',
    index=9,
    containing_service=None,
    input_type=_FAVORITEREQUEST,
    output_type=_BASEREPLY,
    serialized_options=b'\202\323\344\223\002\032\"\025/v1/common/unfavorite:\001*',
  ),
  _descriptor.MethodDescriptor(
    name='Address',
    full_name='bitsflea.BitsFlea.Address',
    index=10,
    containing_service=None,
    input_type=_ADDRESSREQUEST,
    output_type=_BASEREPLY,
    serialized_options=b'\202\323\344\223\002\027\"\022/v1/common/address:\001*',
  ),
  _descriptor.MethodDescriptor(
    name='UpdateAddress',
    full_name='bitsflea.BitsFlea.UpdateAddress',
    index=11,
    containing_service=None,
    input_type=_ADDRESSREQUEST,
    output_type=_BASEREPLY,
    serialized_options=b'\202\323\344\223\002\035\"\030/v1/common/updateaddress:\001*',
  ),
  _descriptor.MethodDescriptor(
    name='DelAddress',
    full_name='bitsflea.BitsFlea.DelAddress',
    index=12,
    containing_service=None,
    input_type=_ADDRESSREQUEST,
    output_type=_BASEREPLY,
    serialized_options=b'\202\323\344\223\002\032\"\025/v1/common/deladdress:\001*',
  ),
  _descriptor.MethodDescriptor(
    name='SetDefaultAddr',
    full_name='bitsflea.BitsFlea.SetDefaultAddr',
    index=13,
    containing_service=None,
    input_type=_SETDEFAULTADDRREQUEST,
    output_type=_BASEREPLY,
    serialized_options=b'\202\323\344\223\002\032\"\025/v1/common/setdefault:\001*',
  ),
  _descriptor.MethodDescriptor(
    name='Upload',
    full_name='bitsflea.BitsFlea.Upload',
    index=14,
    containing_service=None,
    input_type=_FILEREQUEST,
    output_type=_BASEREPLY,
    serialized_options=None,
  ),
])
_sym_db.RegisterServiceDescriptor(_BITSFLEA)

DESCRIPTOR.services_by_name['BitsFlea'] = _BITSFLEA

# @@protoc_insertion_point(module_scope)
